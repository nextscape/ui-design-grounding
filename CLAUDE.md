...
skills/
  ...
  <command-skill>/             ← 21件のコマンドスキル（ユーザー向けスラッシュコマンド）
...
2. **コマンドスキル**（`skills/<name>/`） — 21 件のユーザー呼び出し可能なスラッシュコマンド（`/design-ui`、`/audit-ui`、`/score-ui`、`/ui-help` 等）。各スキルが構造化されたワークフローを定義し、必要に応じてナレッジベースのリファレンスを参照しながら分析・評価・実装をガイドする。
