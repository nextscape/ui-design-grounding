...
AI エージェントは UI のコードを書けますが、その UI が「良い」かを判断する基準を持ちません。
基準の欠落は「なんとなくダサい」という見た目の問題として表に出ます。
しかし根は同じで、**使いやすさ・アクセシビリティ・一貫性・堅牢性・速度といった、ユーザー体験を左右する品質の判断**がまるごと抜け落ちています。

このプラグインは、その判断軸（なぜそうするか）を AI に与え、判断を効かせたまま実装・修正まで完遂させます。
下した判断は **[DESIGN.md](#designmd--設計の基盤)** という資産に残り、プロジェクトに関わる全員・全ツールに効き続けます。

### 誰に、どんなメリットか

**開発者** — デザインの専門知識がなくても、ビジネスで通用する標準品質の UI を出せる。

- アクセシビリティ・レスポンシブ・パフォーマンス・エッジケースなど「忘れがちな品質」を5軸監査で体系的に担保
- DESIGN.md がデザイン指示の代わりになり、毎回プロンプトに書かなくてよい
- 基準を実装前に効かせるため、レビューでの手戻りを減らせる

**デザイナー** — 頭の中の判断軸を資産化し、AI と開発者に伝播できる。

- 基準を DESIGN.md に固定すれば、デザイナーが1人でもチーム全体・全画面へ意図が行き渡る
- トークン抽出（`extract-ui`）や外部サイト分析（`scan-ui`）でデザインシステムの整備を加速
- すべての提案に理由が明示されるため、「なぜ」をレビューしやすい

**利用者** — 迷わず・誰でも・どの端末でも快適に使える。

- ユーザビリティ評価とワーディング改善で、操作に迷わずエラーから回復できる
- WCAG 準拠のアクセシビリティで、環境や障害の有無を問わず使える
- レスポンシブ対応・堅牢化・パフォーマンス最適化で、どの端末でも崩れず速い
- DESIGN.md 準拠により、画面をまたいでも一貫した体験になる
...
これらを、DESIGN.md を基盤に「基準化 → 考える → 作る → 評価 → 直す → 仕上げ」のライフサイクルで効かせます。
...
コマンドスキルは「**第1層＝動詞（何をするか）**」と「**第2層＝観点（どの軸を実際に直すか）**」で構成されます。

- **第1層（入口・少数の動詞）**: 考える `design-ui` / 作る `implement-ui` / 直す `refine-ui` / 評価する `audit-ui`・`score-ui` / 基準化する `init-design`・`scan-ui`。まずこの動詞だけ覚えれば十分です。
- **第2層（観点・実働ユニット）**: 色・タイポ・レイアウト・モーション・文言・堅牢性・性能などを実際に直すスキル。第1層（`implement` / `refine`）から深掘り時に委譲され、軸が明確なら直接も呼べます。

`implement-ui` / `refine-ui` は、軽微なら自前で直し、深掘りが要る観点だけ第2層へ委譲します。
DESIGN.md を更新できるのは2系統だけです — 全トークンを扱う `init-design` / `scan-ui`（人間の承認のもと）と、色専用の `recolor-ui`。
第2層が色以外のトークン水準を変えたら、自動では書き換えず `init-design` へ誘導します。

> この2系統に再編した設計判断とその根拠は [docs/skill-architecture.md](docs/skill-architecture.md)（ADR）に記録しています。

## DESIGN.md — 設計の基盤

**DESIGN.md** は「リポジトリの視覚的アイデンティティの憲法」です。
色・タイポグラフィ・余白・角丸・深度・モーション・コンポーネントの「正解」を1ファイルに集約した単一情報源で、全スキルが作業前にこれを読みます。

### なぜ基盤なのか

- **永続する記憶**: スキルは呼んだときだけ効く一過性の記憶。DESIGN.md はファイルとして残り毎回読まれるため、結論を固定でき忘れない。
- **重力の中心**: 1つの観点スキルを呼んでも、DESIGN.md 経由で全観点の基準が一度に視野へ入る。「タイポは直したが色コントラストが壊れた」式の視野狭窄を防ぐ。
...
google-labs-code/design.md 仕様（`version: alpha`）準拠で、機械可読トークンの **YAML front matter** と、全観点の基準を一覧する **Summary** ＋ 散文本文で構成します。
「モダンでクリーン」のような抽象語より「1970年代の大学講義ハンドアウト」のような具体的な参照を重視します。
結論のみ・更新型で 150〜250 行に収めるのが目安で、大規模化したらトークンを `tokens.css` / `tokens.json` に外出しします。
形式の詳細仕様は [design-md-spec.md](skills/ui-design-grounding/reference/design-md-spec.md) を参照してください。

- **作る**: `init-design`（既存コード・要件から生成）/ `scan-ui`（外部 URL から逆算）/ `extract-ui`（コードからトークン抽出）
- **更新する**: 修正系スキルは値の乖離を検出したら誘導するだけ（色 → `recolor-ui` / その他 → `init-design`）。憲法の更新は人間の承認のもとで行い、自動では書き換えない。
- **見る**: `preview-ui`（DESIGN.md をほぼ機械的に `preview.html` へ反映してブラウザで視覚確認）
...
リポジトリは2種類のスキルで構成されます。

1. **ナレッジベース** (`skills/ui-design-grounding/`) — 17 件のリファレンス文書に UI/UX 設計の判断基準・原則・パターンを集約。ユーザーは直接呼び出さない。
2. **コマンドスキル** (`skills/<name>/`) — 22 件のスラッシュコマンドが構造化されたワークフローを提供し、ナレッジベースを参照する。「仕組み」で述べた第1層／第2層は、このコマンドスキルの内部区分です。
