...
| `/legibility-ui` | **評価（初見）**：実装を読まず画面だけを見て判断 | 見た目と機能の一致・目的の透明性・スコープ整合・重複表示・画面間一貫性・現在地の把握しやすさの6レンズで監査。Playwrightで実際に操作し、初見の予想と実際の挙動のギャップを記録。コードは指摘特定後の裏付けとしてのみ参照。検出課題を対応スキルへ自動マッピング |
