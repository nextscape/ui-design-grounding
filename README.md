...
> 評価スキル（`audit-ui`, `score-ui`, `legibility-ui`）は検出した問題を対応スキルへ自動マッピングします。
...
skills/
  ui-design-grounding/         ← ナレッジベース（コマンドスキルが参照）
    ...
    reference/                 ← 20件のUI/UX原則・仕様リファレンス
...
1. **ナレッジベース** (`skills/ui-design-grounding/`) — 20 件のリファレンス文書に UI/UX 設計の判断基準・原則・パターンを集約
...
| `ui-report.md` | 評価系スキルのレポート保存先、共通メタ情報、スクリーンショットリンク |
