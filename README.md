...
Claude Code でアプリを実装すると、動くけれど「なんとなくダサい」UI になりがちです。原因は実装力ではなく **デザインの判断軸** の不足。このプラグインは、その判断軸（なぜそうするか）を Claude に与え、**判断を効かせたまま実装・修正まで完遂** させます。

スキルが上乗せするのは次の3つで、その上で実装・修正まで担いきります:

- **判断軸の注入** — 17 件のリファレンスに UI/UX の原則・パターンを集約
