...
まず既存コード資産（CSS / トークン / コンポーネント / テーマ設定）を軽く走査して**抽出確度**を判定し、`interview.md` の原則と実施プロトコル（決定木を依存関係ごとに1問ずつ深掘る・調べれば分かることは聞かない・推奨回答を添える・共通理解まで行動しない）でヒアリングする。**聞けば分かることを無断で仮置きしない**（仮置きは質問しても確定しなかった場合の最後の手段）。
...
ヒアリングと既存コード分析を合わせて、確定事項・推奨で埋める事項・未確定事項を短く要約する。ユーザーが「この理解で進めてよい」と確認してから、DESIGN.md の生成・更新に進む。

### Step 4: front matter（トークン）の構築
...
- 質問しても確定しなかった値のみ仮置きし `/* TODO: 確定待ち */` で明示する（聞けば分かることを無断で仮置きしない）
