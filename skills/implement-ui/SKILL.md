...
- **実装範囲を判定する**: UI に閉じるなら本スキルで完遂。バックエンド・データ層を含むなら UI 部分のみを担う — 接続コード（API 呼び出し・状態管理・モック）までは書き、サーバー側実装・スキーマ変更は範囲外として明示して、全体はプロジェクトの実装ワークフローに委ねる（迷えばユーザーに確認）。
...
- **計画で止めず、実装まで担う**（担うのは自分の実装範囲＝UI 部分。実装プロセス全体は所有せず、バックエンドを勝手に実装しない）。
