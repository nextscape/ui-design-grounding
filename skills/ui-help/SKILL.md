...
| コマンド | 何をするか | こんなとき |
|---------|-----------|-----------|
| `/init-design` | **基準化** — DESIGN.md を生成・更新。design.md 仕様（YAML トークン + Summary + 8セクション散文）準拠。既存CSS/トークンからの抽出と、コードから読めない意図のヒアリングに対応 | デザインシステムの基盤を定義したい |
| `/design-ui` | **考える** — 要件が曖昧ならインタビューで明確化し、UI構造・画面遷移・情報階層・UI状態を設計して `.design/<feature-slug>/FEATURE_DESIGN.md` に保存（DESIGN.md は制約として読むが書かない） | 新しい画面を設計したい |
| `/implement-ui` | **作る** — DESIGN.md と機能設計（あれば）を基準に観点を横断し、第2層へ委譲しながら新規UIを実装まで担う第1層オーケストレーター | デザイン・要件から実装したい |
...
| `/legibility-ui` | **評価（初見）** — 実装を読まず画面だけを見て、見た目と機能の一致・現在地の把握しやすさ・目的の透明性・スコープ整合・重複表示・操作群の構造・画面間一貫性の7レンズで監査。コードは最後に裏付けとしてのみ参照 | 初見のユーザー視点で分かりにくさを洗い出したい |
