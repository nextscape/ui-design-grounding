...
特定した観点を直す。**軽微な逸脱は本スキル内で直し**、専門的な深掘りが要る観点だけ該当の第2層スキルへ委譲する（委譲先は実働ユニットなので、委譲すれば修正が完遂する）。複数観点に逸脱があれば、自前修正または委譲で順に片付ける。`implement-ui` と同じく「軽微＝自前／深掘り＝委譲」の判断で動く。
