...
`design-md-gate.md` の **後段ゲート** を実施する。今回の削減が DESIGN.md のトークン水準・規約（色・装飾・タイポ・モーション・レイアウト等）を変えた場合は乖離を明示し、**色は `/recolor-ui`、その他は `/init-design`** へ誘導する（DESIGN.md は自動では書き換えない）。
