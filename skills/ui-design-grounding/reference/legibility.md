...
- 色の意味の誤用（危険な操作なのに警告色でない、逆に単なる強調が警告色に見えて危険な操作だと誤解させる。例: 元に戻しにくい一括破壊操作〔全て却下・全削除等〕が、生成系ボタンと同色・同じ見た目で並ぶ）
...
- 無効化（グレーアウト）されたコントロールに、なぜ無効なのか・どうすれば有効になるのかの手がかりがない（対象が0件なのか、前提が未達なのか区別がつかない）
...
**問い**: このコントロールが実際に影響する範囲（画面ローカル／機能全体／アプリ全体）と、設置されている場所は一致しているか？逆に、設置されている場所から、何に作用するのかを推測できるか？
...
- 特定の対象（編集中の原稿・一覧の項目等）にだけ効く操作が、対象から離れた共通領域（サイドバー・ヘッダー等）にまとめて置かれ、どのボタンがどの対象に効くのか配置から結べない
- コントロールをまとめるコンテナ（ドロワー・タブ・パネル）の名前と関連しない中身が含まれる
...
- 同じ概念を表すスコープ・状態の語彙が操作ごとに揺れている（例: 「未処理の項目だけを対象にする」という同一概念が「空欄のみ」「未生成」「未提案のみ」と3通りに表記され、同じ意味なのか別の意味なのか初見で判別できない）
...
### ⑥ 操作群の平坦化（Flattened Action Structure）[A-3 情報の整理]

**問い**: 画面内に並ぶ操作群が持つ意味構造（動詞・対象・スコープ・作業手順）を、見た目のグルーピング・並び順から読み取れるか？それとも等質なボタンの羅列に平坦化されているか？

**チェック方法**: 2段階で行う。**(1) 分解の強制**: 見出しや区切りなしに操作コントロールが4個以上連続して並ぶ領域を見つけたら、ラベルの字面（共通接頭辞の有無等）によらず、必ず各操作を「動詞（何をする）／対象（何に）／スコープ（どの範囲に）／性質（生成・編集・破壊・設定）」に分解した表を作る。**(2) 判定**: 分解表に異なる動詞が2つ以上あり、かつ いずれかの動詞にスコープ違いが2つ以上ある（＝動詞×スコープの2次元行列が1次元のリストに潰されている）場合、または操作間に想定される作業手順があるのに並び順・区切りから読み取れない場合に指摘にする。ラベルの共通接頭辞や区切り文字（「動詞: スコープ」等）は平坦化の**傍証**にすぎず、判定基準にしない（命名が不規則でも構造は潰れうる）。

**典型例**:
- 「原稿生成: 空欄のみ」「原稿生成: 全スライド」「音声生成: 全スライド」…のように、動詞×スコープの行列が同列のボタン列に展開され、本来グループ見出しに載るべき動詞がラベルの区切り文字で代用されている
- 生成・編集系の操作と破壊的な操作（全て却下・一括削除等）が、区切りも見た目の差もなく同じ並びに混在している（警告色の欠如としては①でも扱う）
- 原稿作成→音声生成→演出付けのように操作間に想定される作業手順があるのに、並び順・番号・区切りのどれからも順序が読み取れず、最初に押すべき操作が分からない
- グループ化されずに並んだ操作が多く、目的の操作を探すのに全ラベルを読む必要がある

**判断が割れやすいケース**: 操作が2〜3個で動詞が1つしかない場合（例: 「コピー: Markdown」「コピー: HTML」）は、フラットな並びで足りることが多く、見出し付きグループへの昇格はかえってノイズになる。動詞でグループ化したとき各グループに自然な見出し名が付き、探索が明らかに楽になる規模でのみ指摘する。
...
※「配置」に関する気づきでも、何に作用するか・どの順で使うかが読めないという**理解**の問題は④・⑥で本スキルの対象とする。委譲するのは、意味は伝わった上での到達距離・押しやすさ（純粋なエルゴノミクス）のみ。
...
| ⑥操作群の平坦化 | `/arrange-ui`（グルーピング・配置・順序の再設計）、`/design-ui`（操作体系・作業フローの再設計） |
